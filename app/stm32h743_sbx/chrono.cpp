#include "./tick.h"
#include "arch/arm/armv7e_m/mmreg/nvic.h"
#include "arch/arm/armv7e_m/stm32h7/mmreg/rcc.h"
#include "arch/arm/armv7e_m/stm32h7/mmreg/tim2-3-4-5.h"
...
void chrono::steady_clock::init() {
	...
	namespace TIM2 = stm32h7::mmreg::TIM2;
	namespace NVIC = armv7e_m::mmreg::NVIC;
	namespace RCC = stm32h7::mmreg::RCC;

	setBitsMasked(RCC::RCC.APB1LENR.word, RCC::APB1LENRBits::TIM2ENMask, RCC::APB1LENRBits::TIM2ENMask);

	static constexpr uint32_t tim2Freq = 1'000'000;
	static constexpr uint16_t tim2Prescaler = static_cast<uint16_t>(tick::apb1Freq / tim2Freq - 1);
	static_assert(tim2Freq == tick::apb1Freq / (tim2Prescaler + 1));
	...
	static constexpr size_t iserWordIdx = stm32h743::nvic::InterruptNumber::TIM2 / 32;
	static constexpr size_t iserBitIdx = stm32h743::nvic::InterruptNumber::TIM2 % 32;
	setBitsMasked(NVIC::NVIC.ISER[iserWordIdx], (1 << iserBitIdx), (1 << iserBitIdx));
...
void chrono::high_resolution_clock::init() {
	...
	using namespace mmreg;
	namespace TIM3 = stm32h7::mmreg::TIM3;
	namespace NVIC = armv7e_m::mmreg::NVIC;
	namespace RCC = stm32h7::mmreg::RCC;

	setBitsMasked(RCC::RCC.APB1LENR.word, RCC::APB1LENRBits::TIM3ENMask, RCC::APB1LENRBits::TIM3ENMask);

	static constexpr uint32_t tim3Freq = 8'000'000;
	static constexpr uint16_t tim3Prescaler = static_cast<uint16_t>(tick::apb1Freq / tim3Freq - 1);
	static_assert(tim3Freq == tick::apb1Freq / (tim3Prescaler + 1));
	...
	// todo: Explicitly set the highest priority, which is already set by default.
	static constexpr size_t iserWordIdx = stm32h743::nvic::InterruptNumber::TIM3 / 32;
	static constexpr size_t iserBitIdx = stm32h743::nvic::InterruptNumber::TIM3 % 32;
	setBitsMasked(NVIC::NVIC.ISER[iserWordIdx], (1 << iserBitIdx), (1 << iserBitIdx));
